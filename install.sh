...
stage_xbctl() {
    ...
    HAVE_XBCTL=0
    log_warn "未能获取 xbctl；mi-node 服务仍会安装，后续可单独安装 /usr/local/bin/xbctl"
}
...
render_credentials() {
    if [ "${MODE}" = "machine" ]; then
        TOKEN_ENV_KEY="MI_NODE_MACHINE_TOKEN"
    else
        TOKEN_ENV_KEY="MI_NODE_API_TOKEN"
    fi
    {
        echo "# Generated by mi-node install.sh"
        printf '%s=%s\n' "${TOKEN_ENV_KEY}" "$(shell_quote "${TOKEN}")"
    } > "${TMP_DIR}/credentials.env"
    chmod 600 "${TMP_DIR}/credentials.env"
}

append_runtime_config() {
    if [ -z "${GOMEMLIMIT}" ] && [ -z "${GOGC}" ]; then
        return
    fi
    {
        echo "    runtime:"
        [ -n "${GOMEMLIMIT}" ] && echo "      gomemlimit: $(yaml_quote "${GOMEMLIMIT}")"
        [ -n "${GOGC}" ] && echo "      gogc: ${GOGC}"
    } >> "${TMP_DIR}/config.yml"
}

append_egress_config() {
    if [ -z "${EGRESS_SOCKS5}" ] && [ -z "${EGRESS_SHADOWSOCKS_URI}" ]; then
        return
    fi
    {
        echo "      egress:"
        echo "        enable_default_rules: true"
        echo "        prefer_ipv4: true"
        if [ -n "${EGRESS_SOCKS5}" ]; then
            echo "        socks5:"
            echo "          address: $(yaml_quote "${EGRESS_SOCKS5_HOST}")"
            echo "          port: ${EGRESS_SOCKS5_PORT}"
            [ -n "${EGRESS_SOCKS5_USER}" ] && echo "          username: $(yaml_quote "${EGRESS_SOCKS5_USER}")"
            [ -n "${EGRESS_SOCKS5_PASS}" ] && echo "          password: $(yaml_quote "${EGRESS_SOCKS5_PASS}")"
        else
            echo "        shadowsocks:"
            echo "          uri: $(yaml_quote "${EGRESS_SHADOWSOCKS_URI}")"
        fi
    } >> "${TMP_DIR}/config.yml"
}
...
append_cert_config() {
    ...
    {
        echo "    cert:"
        [ -n "${cert_mode}" ] && echo "      cert_mode: $(yaml_quote "${cert_mode}")"
        [ -n "${CERT_DOMAIN}" ] && echo "      domain: $(yaml_quote "${CERT_DOMAIN}")"
        [ -n "${CERT_EMAIL}" ] && echo "      email: $(yaml_quote "${CERT_EMAIL}")"
        [ -n "${CERT_HTTP_PORT}" ] && echo "      http_port: ${CERT_HTTP_PORT}"
        [ -n "${CERT_DNS_PROVIDER}" ] && echo "      dns_provider: $(yaml_quote "${CERT_DNS_PROVIDER}")"
...
# Generated by mi-node install.sh. Edit with care.
instances:
  - panel:
      url: $(yaml_quote "${PANEL_URL}")
      token_env: $(yaml_quote "${TOKEN_ENV_KEY}")
EOF
    if [ "${MODE}" = "machine" ]; then
        cat >> "${TMP_DIR}/config.yml" <<EOF
    machine:
...
EOF
      ...
      node_id: ${NODE_ID}
EOF
        [ -n "${NODE_TYPE}" ] && echo "      node_type: $(yaml_quote "${NODE_TYPE}")" >> "${TMP_DIR}/config.yml"
    fi

    cat >> "${TMP_DIR}/config.yml" <<EOF
    kernel:
      type: $(yaml_quote "${KERNEL_TYPE}")
      config_dir: $(yaml_quote "${INSTALL_ROOT}")
      log_level: $(yaml_quote "${KERNEL_LOG_LEVEL}")
EOF
    append_egress_config
...
EOF
    ...
    if [ "${HEALTH_PORT}" -gt 0 ]; then
        echo "    health_port: ${HEALTH_PORT}" >> "${TMP_DIR}/config.yml"
    fi
    if [ "${DEBUG_PORT}" -gt 0 ]; then
        echo "    debug_port: ${DEBUG_PORT}" >> "${TMP_DIR}/config.yml"
    fi
    chmod 600 "${TMP_DIR}/config.yml"
}

render_meta() {
    local target_key target_value latest_id
    if [ "${MODE}" = "machine" ]; then
        target_key="machine_id"
        target_value="${MACHINE_ID}"
        latest_id="machine-${MACHINE_ID}"
    else
        target_key="node_id"
        target_value="${NODE_ID}"
        latest_id="node-${NODE_ID}"
    fi
    cat > "${TMP_DIR}/install-meta.json" <<EOF
{
  "config_mode": "instances",
  "version": $(json_quote "${RELEASE_VERSION}"),
  "latest_instance_id": $(json_quote "${latest_id}"),
  "instance_count": 1,
  "instances": [
    {
      "id": $(json_quote "${latest_id}"),
      "panel_url": $(json_quote "${PANEL_URL}"),
      "mode": $(json_quote "${MODE}"),
      "${target_key}": ${target_value},
      "health_port": ${HEALTH_PORT}
    }
  ],
  "updated_at": $(json_quote "$(date -u '+%Y-%m-%dT%H:%M:%SZ')")
}
EOF
...
require_reconfigure_confirmation() {
    ...
    echo ""
    log_warn "检测到已有 mi-node 安装。继续会备份并覆盖 ${CONFIG_FILE}、${CREDENTIALS_FILE} 和 ${SERVICE_PATH}。"
